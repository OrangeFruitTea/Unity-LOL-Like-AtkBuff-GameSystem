...
| 关联文档 | **`毕设演示-端到端场景与界面流转.md`**、`对局时间模块设计文档.md`、`工程模块与命名空间-现行实现总览.md`、`局内输入与NewInputSystem接轨设计.md`；[`MOBA水晶与对局终局裁定设计文档.md`](MOBA水晶与对局终局裁定设计文档.md)；网络见 `NetworkSyncService.md`（扩展） |
...
|----|----------|------------|-------------------|
| **V1** | 摧毁敌方 **基地 / 核心建筑**（ECS 详见 [`MOBA水晶与对局终局裁定设计文档.md`](MOBA水晶与对局终局裁定设计文档.md)） | 建筑实体死亡事件或血量 ≤ 0 | 高（核心玩法） |
